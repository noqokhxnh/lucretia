...
MusicService::MusicService(QObject* parent) : QObject(parent) {
    manager = new QNetworkAccessManager(this);
}

QJsonObject MusicService::fetchState() {
...
void MusicService::handleControl(const QString& action, const QString& arg1, const QString& arg2) {
...
QJsonObject MusicService::getEqState() {
...
void MusicService::setEqBand(const QString& idx, const QString& val) {
...
void MusicService::applyPreset(const QString& name) {
    QJsonObject obj;
...
void MusicService::applyEq() {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        QJsonObject band;
        band["frequency"] = static_cast<double>(freqs[i]);
        band["gain"] = gain;
        band["mode"] = "Bell";
        band["mute"] = false;
        band["q"] = 1.0;
...
QString MusicService::getRunDir() {
...
QVariant MusicService::getProperty(QDBusInterface &iface, const QString &prop) {
...
void MusicService::processImage(const QString &input, const QString &outputBlur, const QString &outputGrad, const QString &outputText, MusicState *data) {
    QImage img;
    ...
    for (int y = 0; y < blurred.height(); ++y) {
        for (int x = 0; x < blurred.width(); ++x) {
            ...
            int r = qBound(0, static_cast<int>((c.red() - 128) * 0.9 + 128 - 30), 255);
            int g = qBound(0, static_cast<int>((c.green() - 128) * 0.9 + 128 - 30), 255);
            int b = qBound(0, static_cast<int>((c.blue() - 128) * 0.9 + 128 - 30), 255);
...
MusicState MusicService::fetchDataInternal() {
    MusicState data;
    ...
    if (!playerService.isEmpty()) {
        ...
        if (!metadata.isEmpty()) {
            ...
            if (data.length > 0) {
                data.lengthStr = QString("%1:%2").arg(static_cast<int>(data.length) / 60, 2, 10, QChar('0')).arg(static_cast<int>(data.length) % 60, 2, 10, QChar('0'));
            } else {
                data.lengthStr = "00:00";
            }
            
            data.positionStr = QString("%1:%2").arg(static_cast<int>(data.position) / 60, 2, 10, QChar('0')).arg(static_cast<int>(data.position) % 60, 2, 10, QChar('0'));
            data.timeStr = data.positionStr + " / " + data.lengthStr;
            
            if (data.length > 1) {
                data.percent = static_cast<int>((data.position * 100.0) / data.length);
            } else {
                data.percent = 0;
            }
