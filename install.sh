...
if [ -f "$TARGET_CONFIG_DIR/bin/quickshell/calendar/.env" ]; then
    mkdir -p "$BACKUP_DIR"
    cp "$TARGET_CONFIG_DIR/bin/quickshell/calendar/.env" "$BACKUP_DIR/weather.env"
fi
...
if [ -f "$(pwd)/install.sh" ] && [ -d "$(pwd)/.git" ] && [ "$(pwd)" != "$HOME" ]; then
    ...
    if ! git clone -b "$TARGET_BRANCH" "$REPO_URL" "$TARGET_CONFIG_DIR"; then
        ...
        if [ -d "$BACKUP_DIR/niri" ]; then
            echo -e "${C_YELLOW}  -> Restoring backup configuration...${RESET}"
            mv "$BACKUP_DIR/niri" "$TARGET_CONFIG_DIR"
        fi
        exit 1
    fi
