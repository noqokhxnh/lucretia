...
echo "[$(date)] Screenshot script called with args: $@" >> "$LOG_FILE"

# Ensure pactl can connect to PipeWire/PulseAudio regardless of launch context
export XDG_RUNTIME_DIR="/run/user/$(id -u)"
export PULSE_RUNTIME_PATH="$XDG_RUNTIME_DIR/pulse"

# Ultra-fast File Manager Launcher using D-Bus activation.
# This opens the file manager instantly (0ms process startup latency)
# and automatically highlights/selects the exact file.
open_in_file_manager() {
    local filepath="$1"
    local dirpath="$(dirname "$filepath")"
    echo "[$(date)] open_in_file_manager called with: $filepath" >> "$LOG_FILE"
    
    # Use standard D-Bus FileManager1 to highlight/select the exact file.
    # This works beautifully on Nautilus, Dolphin, Thunar, Nemo, PCManFM-Qt, etc.
    if dbus-send --session --type=method_call --print-reply --dest=org.freedesktop.FileManager1 \
        /org/freedesktop/FileManager1 \
        org.freedesktop.FileManager1.ShowItems \
        array:string:"file://$filepath" string:"" >> "$LOG_FILE" 2>&1; then
        echo "[$(date)] Successfully triggered D-Bus FileManager1 ShowItems" >> "$LOG_FILE"
        return 0
    fi
    
    # Fallback: Standard folder open
    echo "[$(date)] D-Bus FileManager1 failed. Falling back to xdg-open..." >> "$LOG_FILE"
    xdg-open "$dirpath" >> "$LOG_FILE" 2>&1 &
}
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --beautify) BEAUTIFY_MODE=true; shift ;;
        --no-beautify) BEAUTIFY_MODE=false; shift ;;
    ...
    if [ -f "$FINAL_FILE" ]; then
        (
            ...
            if [ "$ACTION" = "default" ]; then
                open_in_file_manager "$FINAL_FILE"
            fi
        ) &
    else
...
if [ "$FULL_MODE" = true ] || [ -n "$GEOMETRY" ]; then
    ...
    if [ -n "$GEOMETRY" ]; then
        ...
        GRIM_CMD="grim -s $SCALE_FACTOR -l 0 -g \"$GEOMETRY\""
    fi
    GRIM_CMD="$GRIM_CMD -"

    if [ "$EDIT_MODE" = true ]; then
        if ! eval "$GRIM_CMD" | GSK_RENDERER=gl satty --filename - --output-filename "$FILENAME" --init-tool brush --copy-command wl-copy; then
            echo "[$(date)] ERROR: satty or grim failed" >> "$LOG_FILE"
            notify-send -u critical -a "Screenshot" "Error" "Failed to take or edit screenshot."
            exit 1
        fi
    elif [ "$BEAUTIFY_MODE" = true ]; then
        TMP_IMG="/tmp/screenshot_$$.png"
        if ! eval "$GRIM_CMD" > "$TMP_IMG"; then
            echo "[$(date)] ERROR: grim failed for beautify" >> "$LOG_FILE"
            notify-send -u critical -a "Screenshot" "Error" "Failed to take screenshot."
            rm -f "$TMP_IMG"
            exit 1
        fi
        
        QT_QPA_PLATFORM=offscreen "$(dirname "${BASH_SOURCE[0]}")/quickshell/screenshot/screenshot_backend" beautify "$TMP_IMG" "$FILENAME"
        if [ $? -ne 0 ]; then
             echo "[$(date)] ERROR: screenshot_backend beautify failed" >> "$LOG_FILE"
             notify-send -u critical -a "Screenshot" "Error" "Failed to beautify screenshot."
             rm -f "$TMP_IMG"
             exit 1
        fi
        ...
        if ! eval "$GRIM_CMD" | tee "$FILENAME" | wl-copy; then
            echo "[$(date)] ERROR: grim or wl-copy failed" >> "$LOG_FILE"
            notify-send -u critical -a "Screenshot" "Error" "Failed to save screenshot."
            exit 1
        fi
    fi
...
# ---------------------------------------------------------
QML_PATH="$HOME/.config/niri/bin/quickshell/ScreenshotOverlay.qml"
