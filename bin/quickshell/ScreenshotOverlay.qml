...
PanelWindow {
    ...
    focusable: true
    screen: Quickshell.cursorScreen || (Quickshell.screens.length > 0 ? Quickshell.screens[0] : null)
    ...
    readonly property string xdgPicturesDir: Quickshell.env("XDG_PICTURES_DIR")
    readonly property string saveDir: (xdgPicturesDir !== "" ? xdgPicturesDir : (homeDir + "/Pictures")) + "/Screenshots"
    ...
    Item {
        ...
        Row {
            ...
            AnimWrap {
                ...
                ToolbarBtn { iconTxt: "󰏫"; onClicked: root.executeCapture(true, false, false) }
            }
        ...
        Item {
            ...
            Item {
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: root.executeCapture(false, root.isVideoMode, false)
                }
            }
    ...
    Repeater {
        ...
        delegate: Rectangle {
            ...
            RowLayout {
                ...
                ToolbarBtn { iconTxt: "󰅖"; label: "Close"; isDanger: true; onClicked: root.showQrPopup = false }
            }
        }
    }
    ...
    function executeCapture(openEditor, isRecord, isBeautify) {
        let tmpImg = paths.getRunDir("screenshot") + "/raw_capture.png"
        let cmd = ""
        
        if (isRecord) {
            cmd = `bash ~/.config/niri/bin/screenshot.sh --geometry "${root.geometryString}" --record`
        ...
        cmd = `bash ~/.config/niri/bin/screenshot.sh --geometry "${root.geometryString}"`
