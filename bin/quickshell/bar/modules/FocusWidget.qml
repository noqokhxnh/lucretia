...
Rectangle {
    ...
    Row {
        ...
        spacing: (isSolid && !distinctPills) ? 0 : (barWindow ? barWindow.s(focusWidgetRoot.isCompact ? 5 : 6) : (focusWidgetRoot.isCompact ? 5 : 6))
        ...
        IconButton {
            ...
            accentColor: focusWidgetRoot.isCompact ? Qt.lighter(ThemeBackend.surface0, 1.18) : ((isSolid && !distinctPills) ? "transparent" : ThemeBackend.surface0)
            ...
            onClicked: {
                if (Caching.lucretiaDir) {
                    Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/niri/bin/qs_manager.sh toggle applauncher"])
                }
            }
        }
