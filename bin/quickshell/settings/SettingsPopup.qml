...
Item {
    ...
    function legacyDispatchToLua(dispatcher, command) {
        let c = (command || "").trim();
        switch (dispatcher) {
        case "close":
        case "killactive":
        ...
        case "resize":
        case "resizeactive": {
            let parts = c.split(/\s+/).filter(function(p) { return p.length > 0; });
            let x = parts.length > 0 ? parseInt(parts[0], 10) : 0;
            let y = parts.length > 1 ? parseInt(parts[1], 10) : 0;
            if (isNaN(x)) x = 0;
            if (isNaN(y)) y = 0;
        ...
        case "move":
        case "movewindow":
            return 'hl.dsp.window.move({ direction = "' + c + '" })';
        case "focus":
        case "movefocus":
            return 'hl.dsp.focus({ direction = "' + c + '" })';
        case "workspace": {
            let ws = /^\d+$/.test(c) ? c : ("'" + c + "'");
            return "hl.dsp.focus({ workspace = " + ws + " })";
        }
        case "movetoworkspace":
            return "hl.dsp.window.move({ workspace = " + c + " })";
        default:
            return "";
        }
    }

    function dispatchKeybind(dispatcher, command) {
        let d = (dispatcher || "").trim();
        let c = (command || "").trim();
        if (!d && !c) return;

        if (d === "exec" || d === "exec-once" || d.indexOf("exec") === 0) {
            Quickshell.execDetached(["bash", "-c", c]);
            return;
        }
        if (d === "dispatch" || c.indexOf("hl.dsp.") === 0) {
            Quickshell.execDetached(["hyprctl", "dispatch", d === "dispatch" ? c : c]);
            return;
        }

        let lua = legacyDispatchToLua(d, c);
        if (lua) {
                            ...
                            Flow {
                                ...
                                Repeater {
                                    ...
                                    Rectangle {
                                        ...
                                        MouseArea { id: wsMa; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/niri/bin/qs_manager.sh", wsNum.toString()]) }
                                    }
                                }
                            }
                        }
                            ...
                            ColumnLayout {
                                ...
                                Item {
                                    ...
                                    MouseArea {
                                        ...
                                        onClicked: root.dispatchKeybind(model.dispatcher, model.command)
                                    }
                                }
                                ...
                                ColumnLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        Item {
                                            ...
                                            onActiveFocusChanged: {
                                                if (!activeFocus) { accumulatedMods = []; accumulatedKey = ""; Quickshell.execDetached(["hyprctl", "dispatch", "hl.dsp.submap('reset')"]); }
                                                else { Quickshell.execDetached(["hyprctl", "dispatch", "hl.dsp.submap('passthru')"]); }
                                            }
                                        }
                                    }
