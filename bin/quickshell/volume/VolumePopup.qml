...
Item {
    ...
    // STATE & CONFIG
    // -------------------------------------------------------------------------
    readonly property string scriptsDir: Quickshell.env("HOME") + "/.config/niri/bin/quickshell/volume"
    ...
    Process {
        id: audioPoller
        command: [window.scriptsDir + "/get_audio_state"]
        running: true
    ...
    Item {
        ...
        Rectangle {
            ...
            OrbitBackground {
                color1: window.tabColor
                color2: Qt.lighter(window.tabColor, 1.3)
                opacity1: 0.06
                opacity2: 0.04
                            ...
                            RowLayout {
                                ...
                                Item {
                                    ...
                                    Components.QsSlider {
                                        ...
                                        isMuted: window.activeMute
                                        activeColor: window.tabColor
                                        
                                        onDragStarted: { syncDelay.stop(); window.draggingMaster = true; }
                                        onDragEnded: { syncDelay.restart(); window.draggingMaster = false; audioPoller.running = true; }
                                        onValueChanged: {
                                            if (isDragging) {
                                                window.activeVol = value;
                                                masterCmdThrottle.targetPct = value;
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    Components.QsButton {
                                        ...
                                        icon: model.mute || model.volume === 0 ? "󰖁" : (model.volume > 50 ? "󰕾" : "󰖀")
                                        iconSize: window.s(18)
                                        iconColor: model.mute ? window.overlay0 : window.subtext0
                                        
                                        borderColor: isHovered ? (model.mute ? window.overlay0 : window.tabColor) : "transparent"
                                    ...
                                    Item {
                                        ...
                                        Components.QsSlider {
                                            ...
                                            onDragStarted: { syncDelay.stop(); window.draggingNodes[model.id] = true; }
                                            onDragEnded: { syncDelay.restart(); window.draggingNodes[model.id] = false; audioPoller.running = true; }
                                            onValueChanged: {
                                                if (isDragging) {
                                                    let pct = value;
