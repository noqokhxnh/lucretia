...
Item {
    ...
    function isTargetMonitor(mon) {
        ...
        return (m === safe || m === raw || (scr !== "" && (m === scr || m === scrSafe)));
    }
    ...
    function saveNow() {
        ...
        QsDaemonClient.saveWidgetLayout(loaderRoot.safeMonitorName, data);
        ...
        let daemonTargetDir = Caching.home + "/.local/state/quickshell/widgets/" + loaderRoot.safeMonitorName;
        let saveScript = "mkdir -p '" + targetDir + "' '" + daemonTargetDir + "' && printf '%s' '" + b64Data + "' | base64 -d | tee '" + daemonTargetDir + "/layout.json' > '" + targetFile + "'";
    ...
    Process {
        id: loadProcess
        command: ["bash", "-c", "cat '" + Caching.getStateDir("widgets/" + loaderRoot.safeMonitorName) + "/layout.json' 2>/dev/null || cat '" + Caching.home + "/.local/state/quickshell/widgets/" + loaderRoot.safeMonitorName + "/layout.json' 2>/dev/null || echo '[]'"]
