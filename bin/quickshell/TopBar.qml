...
Variants {
    ...
    delegate: Component {
	    ...
	    Component.onCompleted: {
            ...
            barWindow.musicData = Components.QsDaemonClient.musicState;
	    }	     	
            ...
            readonly property bool showHelpIcon: Config.topbarHelpIcon
            property bool isRecording: false
            
            property bool updateAvailable: false
            property bool forceUpdateShow: false
            property bool isUpdateVisible: (updateAvailable || forceUpdateShow) && (Config.enabledModules["updater"] !== false)
            
            readonly property int workspaceCount: Config.workspaceCount
	    ...
	    Process {
            ...
            // Timer/Pomodoro state from floating widget
            property var timerState: null
            property bool isTimerActive: timerState !== null && timerState.running === true
            property string timerDisplayText: {
                if (!timerState) return "";
                let ms = timerState.remaining || timerState.elapsed || 0;
                let totalSec = Math.floor(ms / 1000);
                let m = Math.floor(totalSec / 60);
                let s = totalSec % 60;
                return m.toString().padStart(2, '0') + ":" + s.toString().padStart(2, '0');
            }
            property string timerIcon: {
                if (!timerState) return "󱎫";
                if (timerState.mode === "pomodoro") {
                    return timerState.phase === 0 ? "󱎫" : "󰒲";
                }
                if (timerState.mode === "stopwatch") return "󱎬";
                return "󰔟";
            }
            property color timerColor: {
                if (!timerState) return mocha.mauve;
                if (timerState.mode === "pomodoro") {
                    if (timerState.phase === 0) return mocha.mauve;
                    if (timerState.phase === 1) return mocha.green;
                    return mocha.blue;
                }
                return mocha.peach;
            }

            readonly property string timerStateFile: {
                let xdg = Quickshell.env("XDG_RUNTIME_DIR");
                return (xdg !== "" ? xdg : "/tmp") + "/qs/timer/state";
            }

            Process {
                id: timerStatePoller
                command: ["bash", "-c", "cat '" + barWindow.timerStateFile + "' 2>/dev/null || echo ''"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
                            try { barWindow.timerState = JSON.parse(txt); } catch(e) { barWindow.timerState = null; }
                        } else {
                            barWindow.timerState = null;
                        }
                    }
                }
            }
            ...
            Process {
                ...
                running: true
                command: ["bash", "-c", "mkdir -p $(dirname '" + barWindow.timerStateFile + "') && exec inotifywait -qq -e modify,close_write,delete '" + barWindow.timerStateFile + "' 2>/dev/null || sleep 2"]
                onExited: {
                    timerStatePoller.running = false;
                    timerStatePoller.running = true;
                    running = false;
                    running = true;
                }
            }
            ...
            onMusicDataChanged: {
                if (musicData && (musicData.status === "Playing" || musicData.status === "Paused") && 
                    musicData.title !== "" && musicData.title !== "Not Playing" && musicData.title !== "Loading...") {
            ...
            property bool isMediaActive: Config.enabledModules["music"] !== false &&
                                         (barWindow.musicData.status === "Playing" || barWindow.musicData.status === "Paused") && 
                                         barWindow.musicData.title !== "Not Playing" && 
                                         barWindow.musicData.title !== "" &&
                                         barWindow.musicData.title !== "Loading..."
		...
		running: true
            ...
            Connections {
                ...
                function onMusicStateReceived(payload) {
                    if (payload) {
                        if (barWindow.userIsSkipping) {
                            if (payload.title !== barWindow.musicData.title && payload.title !== "Loading...") {
                                barWindow.userIsSkipping = false;
                                barWindow.musicData = payload;
                            }
                        } else {
                            barWindow.musicData = payload;
                        }
            ...
            Timer {
                id: skipDebounceTimer
                interval: 2000
                onTriggered: barWindow.userIsSkipping = false
            }

            Process {
                id: kbPoller; running: true
                command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/kb_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
            ...
            Process { id: kbWaiter; command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/kb_wait.sh"]; onExited: (exitCode) => { if (exitCode === 0) { kbPoller.running = false; kbPoller.running = true; } } }

            Process {
                id: audioPoller; running: true
                command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/audio_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: audioWaiter; command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/audio_wait.sh"]; onExited: (exitCode) => { if (exitCode === 0) { audioPoller.running = false; audioPoller.running = true; } } }

            Process {
                id: networkPoller; running: true
                command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/network_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: networkWaiter; command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/network_wait.sh"]; onExited: (exitCode) => { if (exitCode === 0) { networkPoller.running = false; networkPoller.running = true; } } }

            Process {
                id: btPoller; running: true
                command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/bt_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: btWaiter; command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/bt_wait.sh"]; onExited: (exitCode) => { if (exitCode === 0) { btPoller.running = false; btPoller.running = true; } } }

            Process {
                id: batteryPoller; running: true
                command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/battery_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process { id: batteryWaiter; command: ["bash", "-c", "~/.config/niri/bin/quickshell/watchers/battery_wait.sh"]; onExited: (exitCode) => { if (exitCode === 0) { batteryPoller.running = false; batteryPoller.running = true; } } }
            ...
            Item {
                    ...
                    Row {
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle guide"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle applauncher"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle dashboard"])
                            }
                        }
                        ...
                        Rectangle {
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle settings"])
                            }
                        }
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    anchors.fill: parent
                                    onClicked: {
                                        Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh " + wsName])
                                    }
                                }
                            }
                        }
                    ...
                    Item {
                        ...
                        Row {
                            ...
                            MouseArea {
                                ...
                                hoverEnabled: true
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle music"])
                            ...
                            Row {
                                ...
                                Item { 
                                    ...
                                    MouseArea { id: prevMouse; hoverEnabled: true; anchors.fill: parent; onClicked: { 
                                        barWindow.userIsSkipping = true;
                                        skipDebounceTimer.restart();
                                        let temp = Object.assign({}, barWindow.musicData);
                                        temp.title = "Loading..."; temp.percent = 0; temp.timeStr = "00:00 / 00:00";
                                        barWindow.musicData = temp;
                                ...
                                Item { 
                                    ...
                                    MouseArea { id: playMouse; hoverEnabled: true; anchors.fill: parent; onClicked: { Components.QsDaemonClient.sendRequest("music", "control", { command: "play-pause" }); } } 
                                }
                                Item { 
                                    width: barWindow.s(24); height: barWindow.s(24); 
                                    anchors.verticalCenter: parent.verticalCenter
                                    ...
                                    MouseArea { id: nextMouse; hoverEnabled: true; anchors.fill: parent; onClicked: { 
                                        barWindow.userIsSkipping = true;
                                        skipDebounceTimer.restart();
                                        let temp = Object.assign({}, barWindow.musicData);
                                        temp.title = "Loading..."; temp.percent = 0; temp.timeStr = "00:00 / 00:00";
                ...
                Rectangle {
                    ...
                    MouseArea {
                        ...
                        anchors.fill: parent
                        hoverEnabled: true
                        onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle calendar"])
                    }
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        visible: Config.enabledModules["dnd"] !== false
                        radius: barWindow.s(10); height: barWindow.barHeight
                        clip: true
                        ...
                        Rectangle {
                            anchors.fill: parent
                            radius: barWindow.s(10)
                            opacity: Config.dndMode ? 1.0 : 0.0
                            Behavior on opacity { NumberAnimation { duration: 300 } }
                            gradient: Gradient {
                        ...
                        width: visible ? barWindow.barHeight : 0
                        ...
                        property bool initAnimTrigger: false
                        Timer { running: rightContent.showLayout && !dndPill.initAnimTrigger && dndPill.visible; interval: 250; onTriggered: dndPill.initAnimTrigger = true }
                        opacity: initAnimTrigger ? 1 : 0
                        transform: Translate { y: dndPill.initAnimTrigger ? 0 : barWindow.s(15); Behavior on y { NumberAnimation { duration: 500; easing.type: Easing.OutBack } } }
                        Behavior on opacity { NumberAnimation { duration: 400; easing.type: Easing.OutCubic } }
                        ...
                        Text {
                            ...
                            color: Config.dndMode ? mocha.base : mocha.subtext0
                            Behavior on color { ColorAnimation { duration: 200 } }
                        }
                        MouseArea { id: dndMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Config.toggleDnd() }
                    }

                    // Timer/Pomodoro indicator pill
                    Rectangle {
                        id: timerPill
                        property bool isHovered: timerPillMouse.containsMouse
                        visible: barWindow.timerState !== null && (Config.enabledModules["focustime"] !== false)
                        radius: barWindow.s(10); height: barWindow.barHeight
                        clip: true
                        color: isHovered ? Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.6) : Qt.rgba(mocha.surface0.r, mocha.surface0.g, mocha.surface0.b, 0.4)

                        width: visible ? timerPillContent.implicitWidth + barWindow.s(20) : 0
                        ...
                        Rectangle {
                            ...
                            color: barWindow.timerColor
                            opacity: 0.6
                            width: {
                                if (!barWindow.timerState) return 0;
                                if (barWindow.timerState.mode === "stopwatch") return parent.width;
                                let total = barWindow.timerState.total || 1;
                                let remaining = barWindow.timerState.remaining || 0;
                                return parent.width * (1 - remaining / total);
                            }
                        ...
                        Row {
                            ...
                            Text {
                                ...
                                SequentialAnimation on opacity {
                                    running: barWindow.isTimerActive
                                    loops: Animation.Infinite
                            ...
                            Text {
                                text: barWindow.timerDisplayText
                                font.family: "JetBrains Mono"
                                font.pixelSize: barWindow.s(13)
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle focustime"]);
                            }
                        }
                    }
                    ...
                    Rectangle {
                        id: notePill
                        property bool isHovered: noteMouse.containsMouse
                        visible: Config.enabledModules["notes"] !== false
                        radius: barWindow.s(10); height: barWindow.barHeight
                        clip: true
                        color: isHovered ? Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.6) : Qt.rgba(mocha.surface0.r, mocha.surface0.g, mocha.surface0.b, 0.4)
                        ...
                        property bool initAnimTrigger: false
                        Timer { running: rightContent.showLayout && !notePill.initAnimTrigger && notePill.visible; interval: 300; onTriggered: notePill.initAnimTrigger = true }
                        opacity: initAnimTrigger ? 1 : 0
                        transform: Translate { y: notePill.initAnimTrigger ? 0 : barWindow.s(15); Behavior on y { NumberAnimation { duration: 500; easing.type: Easing.OutBack } } }
                        Behavior on opacity { NumberAnimation { duration: 400; easing.type: Easing.OutCubic } }
                        ...
                        MouseArea { id: noteMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle notes"]) }
                    }
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                property real targetWidth: Config.enabledModules["system"] !== false ? kbLayoutRow.implicitWidth + barWindow.s(24) : 0
                                width: targetWidth
                                ...
                                property bool initAnimTrigger: false
                                Timer { running: rightContent.showLayout && !kbLayoutPill.initAnimTrigger; interval: 0; onTriggered: kbLayoutPill.initAnimTrigger = true }
                                opacity: initAnimTrigger ? 1 : 0
                                transform: Translate { y: kbLayoutPill.initAnimTrigger ? 0 : barWindow.s(15); Behavior on y { NumberAnimation { duration: 500; easing.type: Easing.OutBack } } }
                                Behavior on opacity { NumberAnimation { duration: 400; easing.type: Easing.OutCubic } }
                            ...
                            Rectangle {
                                ...
                                property real targetWidth: Config.enabledModules["wifi"] !== false ? wifiLayoutRow.implicitWidth + barWindow.s(24) : 0
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                MouseArea { id: wifiMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle network wifi"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                property real targetWidth: Config.enabledModules["bluetooth"] !== false ? (barWindow.isDesktop ? 0 : btLayoutRow.implicitWidth + barWindow.s(24)) : 0
                                width: targetWidth
                                visible: targetWidth > 0
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                            ...
                            Rectangle {
                                ...
                                property real targetWidth: Config.enabledModules["volume"] !== false ? volLayoutRow.implicitWidth + barWindow.s(24) : 0
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                MouseArea { id: volMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle volume"]) }
                            }
                            ...
                            Rectangle {
                                ...
                                property real targetWidth: Config.enabledModules["battery"] !== false ? (barWindow.isDesktop ? barWindow.s(34) : batLayoutRow.implicitWidth + barWindow.s(24)) : 0
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                MouseArea { id: batMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/qs_manager.sh toggle battery"]) }
                            }                       
                 }
            }
            Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/screenshot.sh"]); 
                            }
                        }
                    }
                }
