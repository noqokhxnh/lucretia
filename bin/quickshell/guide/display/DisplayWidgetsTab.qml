...
Item {
    ...
    Process {
        ...
        running: false
        command: {
            let assetsDir = (typeof Caching !== "undefined" && Caching.assetsDir) ? (Caching.assetsDir + "/widgets") : "";
            let userDir = (typeof Caching !== "undefined" && typeof Caching.getStateDir === "function") ? (Caching.getStateDir("widgets/presets")) : ((Caching.home || "/home/khxnh") + "/.local/state/lucretia/widgets/presets");
            let pyScript =
    ...
    function deleteCustomPreset(presetName) {
        ...
        let userPath = (typeof Caching !== "undefined" && typeof Caching.getStateDir === "function") ? (Caching.getStateDir("widgets/presets")) : ((Caching.home || "/home/khxnh") + "/.local/state/lucretia/widgets/presets");
    ...
    Process {
        id: screenDetector
        running: false
        command: ["bash", "-c", "niri msg -j outputs 2>/dev/null || echo '[]'"]
        stdout: StdioCollector {
            onStreamFinished: {
                let out = this.text;
                if (!out) return;
    ...
    function openRedactor(mon) {
        let runDir = (typeof Caching !== "undefined" && typeof Caching.getRunDir === "function") ? Caching.getRunDir("widgets") : "/tmp/quickshell";
        let mainRunDir = (typeof Caching !== "undefined" && Caching.runDir) ? Caching.runDir : "/tmp/quickshell";
        let runnerTarget = (typeof Caching !== "undefined" && Caching.qsDir) ? (Caching.qsDir + "/Runner.qml") : "";
        let redactorTarget = (typeof Caching !== "undefined" && Caching.qsDir) ? (Caching.qsDir + "/widgets/WidgetRedactor.qml") : "";
        let launchCmd = "{ for p in $(pgrep -f '[R]unner\\.qml'); do [ \"$p\" != \"$$\" ] && [ \"$p\" != \"$PPID\" ] && kill \"$p\" 2>/dev/null; done; mkdir -p '" + runDir + "' '" + mainRunDir + "' && printf '%s' '" + mon + "' > '" + mainRunDir + "/redactor_target_monitor' && printf '%s' '" + mon + "' > '" + runDir + "/redactor_target_monitor' && QS_WIDGET_MONITOR='" + mon + "' LUCRETIA_TARGET_FILE='" + redactorTarget + "' SERPANTINUM_TARGET_FILE='" + redactorTarget + "' quickshell -p '" + runnerTarget + "'; } >/dev/null 2>&1";
        Quickshell.execDetached(["bash", "-c", launchCmd]);
        if (rootObj && typeof rootObj.closePopup === "function") {
            rootObj.closePopup();
        } else {
            Quickshell.execDetached(["bash", (Caching.home || "") + "/.config/niri/bin/qs_manager.sh", "close"]);
        }
    }
