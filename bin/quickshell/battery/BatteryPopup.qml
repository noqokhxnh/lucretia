...
Item {
    ...
    readonly property color maroon: ThemeBackend.maroon
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (lines.length >= 6) {
                    ...
                    if (Config.powerProfile !== lines[2]) {
                        Config.powerProfile = lines[2];
                    }
    ...
    // Expose _notifIdMap from main so we can clean it up on dismiss
    property var notifIdMap: null

    // Helper: Dismiss and close a single live notif by uid (also cleans dedup map)
    function closeLiveNotif(uid) {
        if (window.liveNotifs && window.liveNotifs[uid]) {
            let live = window.liveNotifs[uid];
            // CRITICAL: set tracked = false FIRST so Quickshell releases its
            // internal reference. Without this, after Quickshell.reload() the
            // NotificationServer re-emits every tracked=true notification back
            // through onNotification, causing dismissed notifications to reappear.
            if (typeof live.tracked !== "undefined") {
                live.tracked = false;
            }
        ...
        // Clean dedup map in Main.qml so the same DBus id won't re-insert
        if (window.notifIdMap) {
            for (let key in window.notifIdMap) {
                if (window.notifIdMap[key] === uid) {
                    delete window.notifIdMap[key];
                    break;
                }
            }
        }
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            Components.QsButton {
                                ...
                                activeColor: Qt.alpha(window.red, 0.15)
                                isActive: window.dndEnabled
                                borderColor: window.dndEnabled ? window.red : (isHovered ? window.surface2 : "transparent")
                                
                                icon: window.dndEnabled ? "󰂛" : "󰂚"
                                text: window.dndEnabled ? "Silent" : (isHovered ? "Mute" : "")
                                textSize: window.s(13)
                                iconSize: window.s(18)
                                
                                textColor: window.dndEnabled ? window.red : window.text
                                iconColor: window.dndEnabled ? window.red : (isHovered ? window.text : window.overlay0)
                                ...
                                Rectangle {
                                    ...
                                    RowLayout {
                                        ...
                                        Components.QsButton {
                                            ...
                                            iconColor: isHovered ? window.red : window.overlay0
                                ...
                                function removeThisNotif() {
                                    ...
                                    for (let i = 0; i < window.notifModel.count; i++) {
                                        if (window.notifModel.get(i).uid === model.uid) {
                                            window.closeLiveNotif(model.uid);
                                    ...
                                    ColumnLayout {
                                        ...
                                        RowLayout {
                                            ...
                                            Components.QsButton {
                                                ...
                                                radius: window.s(11)
                                                baseColor: "transparent"
                                                hoverColor: Qt.alpha(window.red, 0.15)
                                        ...
                                        RowLayout {
                                            ...
                                            Repeater {
                                                ...
                                                delegate: Components.QsButton {
                                                    ...
                                                    textSize: window.s(11)
                                                    
                                                    isActive: isPrimary
                                                    activeColor: window.blue
                                                    activeTextColor: window.crust
                                                    
                                                    baseColor: isPrimary ? window.blue : window.surface1
                                                    hoverColor: isPrimary ? Qt.darker(window.blue, 1.2) : window.surface2
                                                    textColor: isPrimary ? window.crust : window.text
                                                    borderColor: isPrimary ? window.blue : window.surface2
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: { 
                                ...
                                Quickshell.execDetached(["bash", "-c", "~/.config/niri/bin/exit.sh"]); 
                    ...
                    Item {
                        ...
                        ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Item {
                                            ...
                                            Components.QsSlider {
                                                ...
                                                activeColor: window.batColorStart
                                                trackColor: window.surface1
                                                borderColor: window.surface2
                                                
                                                onDragStarted: { briSyncDelay.stop(); window.isDraggingBri = true; }
                                                onDragEnded: { briSyncDelay.restart(); window.isDraggingBri = false; }
                                                
                                                onValueChanged: {
                                                    if (isDragging) {
                                                        window.sysBrightness = value;
                                                        briCmdThrottle.targetPct = value;
                                    ...
                                    RowLayout {
                                        ...
                                        Rectangle {
                                            ...
                                            MouseArea {
                                                ...
                                                onClicked: {
                                                    ...
                                                    Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/niri/bin/quickshell/volume/audio_control.sh", "toggle-mute", "sink", "@DEFAULT@"]);
                                        ...
                                        Item {
                                            ...
                                            Timer {
                                                ...
                                                onTriggered: {
                                                    if (targetPct >= 0) {
                                                        ...
                                                        Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/niri/bin/quickshell/volume/audio_control.sh", "set-volume", "sink", "@DEFAULT@", targetPct]);
                                                        targetPct = -1;
                                                    }
                                                }
                                            }
                                            ...
                                            Components.QsSlider {
                                                ...
                                                trackColor: window.surface1
                                                borderColor: window.surface2
                                                mutedColor: window.surface2
                                                
                                                onDragStarted: { volSyncDelay.stop(); window.isDraggingVol = true; }
                                                onDragEnded: { volSyncDelay.restart(); window.isDraggingVol = false; }
                                                
                                                onValueChanged: {
                                                    if (isDragging) {
                                                        window.sysVolume = value;
                                                        volCmdThrottle.targetPct = value;
                            ...
                            RowLayout {
                                ...
                                Repeater {
                                    model: ListModel {
                                        ListElement { cmd: "bash $HOME/.config/niri/bin/lock.sh"; icon: ""; baseColor: "mauve"; weight: 1.0 }
                                        ListElement { cmd: "bash $HOME/.config/niri/bin/lock.sh; systemctl suspend"; icon: "ᶻ 𝗓 𝗓"; baseColor: "blue"; weight: 1.0 }
                                    ...
                                    delegate: Rectangle {
                                        ...
                                        Timer {
                                            ...
                                            property string capsuleCmd: cmd
                                            onTriggered: {
                                                Quickshell.execDetached(["notify-send", "Power Action", "Executing: " + capsuleCmd]);
                                                Quickshell.execDetached(["bash", "-c", capsuleCmd]); 
                                    ...
                                    Repeater {
                                        ...
                                        delegate: Item {
                                            ...
                                            MouseArea {
                                                id: profileMa
                                                anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor
                                                onClicked: { Config.setPowerProfile(name); sysPoller.running = true; }
                                            }
                                        }
                                    }
                                }
