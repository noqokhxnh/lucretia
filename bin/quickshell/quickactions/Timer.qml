...
Item {
    ...
    // State broadcasting to TopBar
    readonly property string timerStateDir: {
        let xdg = Quickshell.env("XDG_RUNTIME_DIR");
        return (xdg !== "" ? xdg : "/tmp") + "/qs/timer";
    }
    ...
    function broadcastState() {
        let state = {};
        if (stateCache.activeMode === 0 && (stateCache.timerTargetEpoch > 0 || (!root.isTimerIdle))) {
            state = { mode: "timer", running: root.isTimerRunning, remaining: stateCache.timerRemainingMs, total: stateCache.timerPresetMs };
        } else if (stateCache.activeMode === 1 && stateCache.swStartEpoch > 0) {
            state = { mode: "stopwatch", running: true, elapsed: stopwatchView.currentDisplayMs };
        } else if (stateCache.activeMode === 2 && (stateCache.pomoTargetEpoch > 0 || stateCache.pomoRemainingMs !== stateCache.pomoWorkLimit * 60 * 1000)) {
            state = { mode: "pomodoro", running: stateCache.pomoTargetEpoch > 0, remaining: stateCache.pomoRemainingMs, phase: stateCache.pomoState, sessions: stateCache.pomoSessionsCount, target: stateCache.pomoTargetSessions };
        }

        if (Object.keys(state).length > 0) {
            stateBroadcast.command = ["bash", "-c", "mkdir -p '" + timerStateDir + "' && echo '" + JSON.stringify(state) + "' > '" + timerStateDir + "/state'"];
        } else {
            stateBroadcast.command = ["bash", "-c", "rm -f '" + timerStateDir + "/state'"];
        }
        stateBroadcast.running = false;
        stateBroadcast.running = true;
    }

    Timer {
        interval: 1000
        running: root.anyTimerActive || (!root.isTimerIdle && stateCache.activeMode === 0) || (stateCache.activeMode === 2)
        repeat: true
        triggeredOnStart: true
        onTriggered: root.broadcastState()
    }
