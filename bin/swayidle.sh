...
# Aggressive power-saving timeouts when running on battery
if [ "$AC_STATUS" = "0" ]; then
    # Battery mode: Screen off 3m (180s), Lock 5m (300s), Suspend 15m (900s)
    [ "$SCREEN_OFF_SEC" -gt 180 ] && SCREEN_OFF_SEC=180
    [ "$LOCK_SEC" -gt 300 ] && LOCK_SEC=300
    [ "$SLEEP_SEC" -gt 900 ] && SLEEP_SEC=900
fi

# Ensure sensible minimum ordering
[ "$SCREEN_OFF_SEC" -lt 30 ] && SCREEN_OFF_SEC=30
[ "$LOCK_SEC" -le "$SCREEN_OFF_SEC" ] && LOCK_SEC=$((SCREEN_OFF_SEC + 60))
[ "$SLEEP_SEC" -le "$LOCK_SEC" ] && SLEEP_SEC=$((LOCK_SEC + 300))

echo "[swayidle] Starting with timeouts: ScreenOff=${SCREEN_OFF_SEC}s, Lock=${LOCK_SEC}s, Suspend=${SLEEP_SEC}s (AC=${AC_STATUS})"

exec swayidle -w \
  timeout "$SCREEN_OFF_SEC" 'niri msg action power-off-monitors' resume 'niri msg action power-on-monitors' \
  timeout "$LOCK_SEC" 'loginctl lock-session' \
  timeout "$SLEEP_SEC" 'systemctl suspend' \
